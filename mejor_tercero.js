...
function mejor_tercero(grupos) {
  if (!Array.isArray(grupos) || grupos.length !== 8) {
    throw new Error("Se necesitan exactamente 8 grupos.");
  }

  const validos = ["A","B","C","D","E","F","G","H","I","J","K","L"];
  for (const g of grupos) {
    if (!validos.includes(g.toUpperCase())) {
      throw new Error(`Grupo inválido: "${g}"`);
    }
  }

  const key = grupos.map(g => g.toUpperCase()).sort().join("");
...
console.log(mejor_tercero(["L","C","B","H","E","G","F","D"]));

module.exports = { mejor_tercero };
