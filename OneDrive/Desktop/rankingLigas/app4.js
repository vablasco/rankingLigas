...
const render = (
  ...
  svg
    ...
    .text(semana => {
      let filterr = data.filter(d => d.semana == semana && d.vs != 'none' && d.goles_fecha !== not_played_yet)
      return filterr.length > 0 ? (d3.sum(filterr, d => d.pts_fecha > 1 ? 1 : 0)) + (' ('+d3.format('.0%')(d3.sum(filterr, d => d.pts_fecha > 1 ? 1 : 0) / (filterr.length/2))+')').replace('.', ',') : ''
    }); */
    ...
    dates.slice(0).forEach((o, i) => {
      ...
      if (yearSlice1.find((d) => d.name == nombre).vs != "none") {
        ...
        names_filter.forEach((team, i) => {
          ...
          let hor = (
            ...
            (i == 0 ? heightBars*0.38 : i == 2 ? -heightBars*0.38 : 0) 
            : 0
          )
      ...
      svg.append("image").attrs({
        ...
        x:
          x(wks) +
          heightBars * 0.125 -
          (defaults.mini_logo.size+ defaults.subValue.style.font_size*0.35) / 2 + weeks * 0.65,
    ...
    .call((text) =>
      text
        ...
        .attrs({
          ...
          data.filter((e) => e.final != true),
          (e) => e.l_or_v == 'L' && e.pts_fecha > 1 && e.goles_fecha !== not_played_yet ? 1 : 0
        )/((d3.sum(lastSlice, (d) => d.partidos_jugados) + d3.sum(lastSlice, (d) => d.partidos_jugados1)) / 2)) + ') ').toString().length*margin.top * 0.2*0.55
        })
        .styles({
          opacity: 1,
          fill: victoria_color,
    ...
    .call((text) =>
      text
        ...
        .text((d) => '' + d3.sum(
          ...
          data.filter((e) => e.final != true),
          (e) => e.l_or_v == 'L' && e.goles_fecha !== not_played_yet ? e.goles_fecha : 0
        )/((d3.sum(lastSlice, (d) => d.partidos_jugados) + d3.sum(lastSlice, (d) => d.partidos_jugados1)) / 2)).replace(".", ",") + ') '
      )
    )
    ...
    .call((text) =>
      text
        ...
        .attrs({
          ...
          data.filter((e) => e.final != true),
          (e) => e.l_or_v == 'L' && e.goles_fecha !== not_played_yet ? e.goles_fecha : 0
        )/((d3.sum(lastSlice, (d) => d.partidos_jugados) + d3.sum(lastSlice, (d) => d.partidos_jugados1)) / 2)).replace(".", ",") + ') ').toString().length*margin.top * 0.2*0.505
        })
        .styles({
          opacity: 1,
          fill: 'lightgrey',
  ...
  svg
    ...
    /* let positions = (array) => {
      ...
      let positions = {
        ...
        2: [[0, 0], [0, 1]],
        3: [[0, 0], [0, 1], [0, 2]],
        4: [[-0.5, 0], [-0.5, 1], [0.5, 0], [0.5, 1]],
        5: [[-0.5, 0], [-0.5, 1], [0.5, 0], [0.5, 1], [0, 2]],
        6: [[-0.5, 0], [-0.5, 1], [-0.5, 2], [0.5, 0], [0.5, 1], [0.5, 2]],
        7: [[-1, 0], [0, 0], [1, 0], [-1, 1], [0, 1], [1, 1], [0, 2]],
        8: [[-1, 0], [0, 0], [1, 0], [-1, 1], [0, 1], [1, 1], [-0.5, 2], [0.5, 2]],
        9: [[0, 0], [0, 1], [0, 2], [-1, 0], [-1, 1], [-1, 2], [1, 0], [1, 1], [1, 2]],
      }

    console.log(positions[2][1][1])
    ...
    svg
      ...
      .attrs({
        ...
        x: d => {
          ...
          ee == 'valla_invicta' ? length++ : 0;length = length + (d3.max(data, (e) => e[ee]).toString().length)})
      ...
      .attrs({
        ...
        x: (d) => {
          ...
          ee == 'fechas_en_top' ? length++ : 0;length = length + (d3.max(data, (e) => e[ee]).toString().length)})
          return width -
          margin_right +
    ...
    .attrs({
      x: (d, i, total) => {
        ...
        return x((dates.length-1) - ((dates.length-1)-fechas_not_played)*not_played_yet_x) + (fechas_not_played<dates.length-1?x(1*not_played_yet_x):0) + margin_left*2 + defaults.logo.size / 2 + defaults.name.position.x + length * heightBars*0.225 + index * heightBars*0.4 + (positions[total.length][i][0]*defaults.mini_logo.size*0.65) - defaults.mini_logo.size/2 + ((d3.max(data, (e) => e[p]).toString().length+1)/2 * heightBars*0.225)
      },
      y: (d, i, total) => y(-1) - heightBars / 2 - (positions[total.length][i][1]*defaults.mini_logo.size*0.7),
      height: defaults.mini_logo.size,
      href: d => `./escudos/${d.name}.png`,
    });
