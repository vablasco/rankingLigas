...
let teamColorss = {
  ...
  usa: ["#FB0306", "#FB0306", "#FB0306", "#FB0306"],
  germany: ["#FB0306", "#FB0306", "#FB0306", "#FB0306"],
  ...
  Huracán: ["#fff", "#fff", "#FB0306", "#fff"],
  "Barracas Central": ["#fff", "#FB0306", "#FB0306", "#fff"],
  "San Martín (T)": ["#FB0306", "#fff", "#fff", "#FB0306"],
...
const excepciones = {
  "indepmedellin":  "Independiente Medellín",
  "indepdelvallle": "Independiente del Valle",
  "indeprivadavia": "Independiente Rivadavia",
  "deplagguaira":   "Deportivo La Guaira",
  "deptolima":      "Deportes Tolima",
  "coquimbounido":  "Coquimbo Unido",
  "ldquito":        "Liga de Quito",
  "univcatolica":   "Universidad Católica",
  "univcentral":    "Universidad Central",
  "rosariocentral": "Rosario Central",
  "cerroporteno":   "Cerro Porteño",
  "alwaysready":    "Always Ready",
  "barcelonaec":    "Barcelona SC",
...
function getPng(slug) {
  const sinGrupo = slug.replace(/-[A-H]$/, '');
  const slugNorm = normalizar(sinGrupo);
  const nombre = excepciones[slugNorm] ?? sinGrupo.replace(/([a-z])([A-Z])/g, '$1 $2');
  return `./escudos/${nombre}.png`;
}
...
/* function getPng(slug) {
  ...
  const nombreSinTildes = nombre
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '');
  return `./escudos/${nombreSinTildes}.png`;
} */

  /* console.log(getPng('Boca Juniors-A')) */

// Ejemplos:
// getPng("IndepMedellin-A") → "./escudos/Independiente Medellin.png"
// getPng("CerroPorteño-F")  → "./escudos/Cerro Porteno.png"
// getPng("UnivCatolica-D")  → "./escudos/Universidad Catolica.png"
// getPng("BocaJuniors-D")   → "./escudos/Boca Juniors.png"
// Ejemplos:
// getPng("BocaJuniors-D")   → "./escudos/Boca Juniors.png"
// getPng("IndepMedellin-A") → "./escudos/Independiente Medellín.png"
// getPng("Flamengo-A")      → "./escudos/Flamengo.png"
...
const render = (
  ...
  let partidos_restantes_chile = data.filter(
    (d) => d.name == "Chile" && d.goles_fecha == 99,
  );
  let winRate = 0;
  partidos_restantes_chile.forEach((d) => {
    let puntos_rival =
      d3.max(
        data.filter((e) => e.name == d.vs),
        (e) => e.value,
      ) / d.value;
  ...
  let ticks_slice =
    ress_ratio == "16:9"
      ? top_n - 4
      : ress_ratio == "1:1"
        ? +d3.format(".0f")(top_n * 0.4)
        : +d3.format(".0f")(top_n * 0.25);
  ...
  fechas_not_played = d3.max(
    data,
    (d) => d.partidos_jugados + d.partidos_jugados1,
  )

  /* fechas_not_played = fechas_not_played + 1; */

  if (grupos > 1) {
    width =
      weeks * (fechas_not_played + 1) +
      (weeks - weeks * not_played_yet_x) *
        (dates.length - 1 - fechas_not_played) +
      margin_right +
  ...
  let sort_teams = (array) => {
    ...
    array.forEach(
      (d, i) => (d.position = d.rankInGroup + 1 + d.name.split("-")[1]),
    );
  ...
  if (grupos > 1) {
    grupos1.forEach((e, index) => {
      svg
        ...
        .styles({
          fill: (d, i) =>
            i <= equipos_por_grupos - 1
              ? i <= clasificacion_por_grupo - 1 ||
                (i >= equipos_por_grupos &&
                  i <= equipos_por_grupos + clasificacion_por_grupo - 1)
                ? i % 2 == 1
                  ? "#90EE90"
                  : "#5da15d"
                : i % 2 == 1
    ...
    rondas.forEach((pp, ppi) => {
      svg
        ...
        .attrs({
          ...
          x:
            width -
            (width_playoffs * (rondas_playoff - ppi) +
              space_width_playoff * (rondas_playoff - ppi)) +
            width_playoffs / 2,
          y: margin.top * 0.8,
        })
        .styles({
    ...
    arr.forEach((dd) => {
      arr_w.forEach((ee, ii) => {
        svg
          .selectAll(".path")
          .data(
            fechas_playoff.filter(
              (d) => d.fecha == `Fecha 1/${primera_ronda_playoff}`,
            ),
          )
          .enter()
          .append("path")
          ...
          .styles({
            opacity: (d, i) =>
              d.goles_local > d.goles_visitante
                ? 1
                : d.goles_local == d.goles_visitante
                  ? d.penales_local > d.penales_visitante
                    ? 1
                    : 1
                  : 1,
            fill: "none",
            stroke: (d) =>
              teamColorss[d[dd].split("-")[0]] == undefined
                ? "grey"
                : teamColorss[d[dd].split("-")[0]][ii],
            ...
            d3.line().curve(d3.curveCardinal.tension(1))([
              [
                ...
                yPlayoffs.domain([primera_ronda_playoff, 0])(
                  positions_playoffs[d["position_" + dd]][0],
                ) +
                  (top_n * heightBars) / primera_ronda_playoff / 2 -
                  heightBars / 2 +
                  (positions_playoffs[d["position_" + dd]][1] == 0
            ...
            d3.line().curve(d3.curveCardinal.tension(1))([
              ...
              [
                width -
                  ...
                  space_width_playoff -
                  space_width_playoff / 3,
                yPlayoffs.domain([primera_ronda_playoff / 2, 0])(
                  positions_playoffs4[
                    positions_playoffs[d["position_" + dd]][0]
                  ][0],
                ) +
                  (top_n * heightBars) / primera_ronda_playoff -
                  heightBars / 2 +
                  (positions_playoffs4[
                    positions_playoffs[d["position_" + dd]][0]
                  ][1] == 0
        ...
        svg
          ...
          .data(
            fechas_playoff.filter(
              (d) => d.fecha == `Fecha 1/${primera_ronda_playoff / 2}`,
            ),
          )
          .enter()
          .append("path")
            ...
            d3.line().curve(d3.curveCardinal.tension(1))([
              ...
              [
                width -
                  ...
                  space_width_playoff +
                  width_playoffs +
                  space_width_playoff * 2 +
                  width_playoffs -
                  space_width_playoff / 3,
                yPlayoffs.domain([primera_ronda_playoff / 4, 0])(
                  positions_playoffs2[
                    positions_playoffs4[
                      positions_playoffs[d["position_" + dd]][0]
                    ][0]
        ...
        svg
          ...
          .data(
            fechas_playoff.filter(
              (d) => d.fecha == `Fecha 1/${primera_ronda_playoff / 4}`,
            ),
          )
          .enter()
          .append("path")
            ...
            d3.line().curve(d3.curveCardinal.tension(1))([
              ...
              [
                width -
                  ...
                  width_playoffs * 2 -
                  space_width_playoff / 3,
                yPlayoffs.domain([primera_ronda_playoff / 8, 0])(
                  positions_playoffs1[
                    positions_playoffs2[
                      positions_playoffs4[
                        positions_playoffs[d["position_" + dd]][0]
                  ...
                  (top_n * heightBars) / (primera_ronda_playoff / 4) -
                  heightBars / 2 +
                  (positions_playoffs1[
                    positions_playoffs2[
                      positions_playoffs4[
      ...
      svg
        ...
        .data(
          fechas_playoff
            .filter((d) => d.fecha == `Fecha 1/${primera_ronda_playoff / 2}`)
            .slice(0, primera_ronda_playoff),
        )
        .enter()
        .append("text")
        .attrs({
      ...
      svg
        ...
        .data(
          fechas_playoff
            .filter((d) => d.fecha == `Fecha 1/${primera_ronda_playoff / 4}`)
            .slice(0, primera_ronda_playoff),
        )
        .enter()
        .append("text")
        .attrs({
          ...
          y: (d, i) =>
            ...
            heightBars / 2 +
            (positions_playoffs2[
              positions_playoffs4[positions_playoffs[d["position_" + dd]][0]][0]
            ][1] == 0
              ? -space_height_playoff
              : space_height_playoff),
        })
      ...
      svg
        ...
        .data(
          fechas_playoff
            .filter((d) => d.fecha == `Fecha 1/${primera_ronda_playoff / 8}`)
            .slice(0, primera_ronda_playoff),
        )
        .enter()
        .append("text")
  ...
  let fechasNotPlayed = (i) => {
    ...
    let b = () => {
      if (i > fechas_not_played) {
        return (
          x(fechas_not_played) -
          x(fechas_not_played - not_played_yet_x * fechas_not_played)
        );
      } else {
        return 0;
      }
  ...
  svg
    ...
    .text((semana) => {
      ...
      return (
        filterr.find((d) => d.year == min_day).dia_large.split(" ")[0] +
        " " +
        min_day.slice(2, 4)
      );
    });
  ...
  svg
    ...
    .attrs({
      class: "years",
      x: (d, i) =>
        fechasNotPlayed(i) /* - heightBars * 0.3 */ -
    ...
    .text((semana) => {
      let filterr = data.filter(
        (d) =>
          d.semana == semana &&
          d.vs != "none" &&
          d.goles_fecha !== not_played_yet,
      );
  ...
  svg
    ...
    .text((semana) => {
      ...
      return filterr.length > 0
        ? (
            "(" +
            d3.format(",.1f")(
              d3.sum(filterr, (d) => d.goles_fecha) / (filterr.length / 2),
            ) +
            ")"
          ).replace(".", ",")
        : "";
    ...
    svg
      ...
      .call((text) =>
        text
          ...
          .text(
            d3.format(".0f")(
              (d3.sum(lastSlice, (d) => d.partidos_jugados) +
                d3.sum(lastSlice, (d) => d.partidos_jugados1)) /
                2,
            ) + " ",
          ),
      )
      ...
      .call((text) =>
        text
          ...
          .text(
            d3.sum(lastSlice, (d) => d.partidos_ganados) +
              d3.sum(lastSlice, (d) => d.partidos_ganados1) +
              " ",
          ),
      )
      ...
      .call((text) =>
        text
          ...
          .text(
            d3.format(".0f")(
              (d3.sum(lastSlice, (d) => d.partidos_empatados) +
                d3.sum(lastSlice, (d) => d.partidos_empatados1)) /
                2,
            ) + " ",
          ),
      )
        ...
        text
          ...
          .text(
            "(" +
              d3
                .format(".1f")(
                  (d3.sum(lastSlice, (d) => d.goles) +
                    d3.sum(lastSlice, (d) => d.goles1)) /
                    d3.format(".0f")(
                      (d3.sum(lastSlice, (d) => d.partidos_jugados) +
                        d3.sum(lastSlice, (d) => d.partidos_jugados1)) /
                        2,
          ...
          .text(
            (d) =>
              ...
              d3.sum(
                data.filter((e) => e.final != true),
                (e) =>
                  e.l_or_v == "L" && e.goles_fecha !== not_played_yet
                    ? e.goles_fecha
                    : 0,
              ) +
              " (" +
          ...
          .text(
            (d) =>
              ...
              d3.sum(
                data.filter((e) => e.final != true),
                (e) =>
                  e.l_or_v == "V" && e.goles_fecha !== not_played_yet
                    ? e.goles_fecha
                    : 0,
              ) +
              " (" +
          ...
          .text(
            (d) =>
              ...
              d3.sum(
                data.filter((e) => e.name == d.name && e.final != true),
                (e) =>
                  e.l_or_v == "L" && e.goles_fecha !== not_played_yet ? 1 : 0,
              ),
          ),
      )
          ...
          .text(
            (d) =>
              ...
              d3.sum(
                data.filter((e) => e.name == d.name && e.final != true),
                (e) =>
                  e.l_or_v == "V" && e.goles_fecha !== not_played_yet ? 1 : 0,
              ),
          ),
      )
          ...
          .styles({
            ...
            fill: (d) =>
              d3.sum(
                ...
                (e) =>
                  e.l_or_v == "L" && e.goles_fecha !== not_played_yet
                    ? e.goles_fecha - e.goles_en_contra_fecha
                    : 0,
              ) > 0
                ? victoria_color
                : d3.sum(
          ...
          .styles({
            ...
            fill: (d) =>
              d3.sum(
                data.filter((e) => e.name == d.name && e.final != true),
                (e) =>
                  e.l_or_v == "V" ? e.goles_fecha - e.goles_en_contra_fecha : 0,
              ) > 0
                ? victoria_color
                : d3.sum(
                      data.filter((e) => e.name == d.name && e.final != true),
  ...
  array_p.forEach((p, index, total) => {
    svg
      .selectAll(".img")
      .data(
        removeDuplicates(data.filter((d) => d[p] == d3.max(data, (d) => d[p]))),
      )
      .enter()
      .append("image")
      .attrs({
        x: (d, i, total) => {
          ...
          return (
            ...
            defaults.mini_logo.size / 2 +
            ((d3.max(data, (e) => e[p]).toString().length + 1) / 2) *
              heightBars *
              0.225
          );
        },
        y: (d, i, total) =>
          y(-1) -
          heightBars / 2 -
          try_positions(total.length, i, 1) * defaults.mini_logo.size * 0.7,
        height: defaults.mini_logo.size,
        href: (d) => `./escudos/${d.name.split("-")[0]}.png`,
      });
...
data1 = agregarSufijoDeGrupo(data1, detectarGrupos(data1)) */
  ...
  /* function ordenarTabla1(tabla) {
    return Object.keys(tabla).sort((a, b) => {
      ...
      // 5. Sorteo (aleatorio, pero fijo por escenario — ver nota abajo)
      return tabla; // en simulación lo tratamos como 50/50 (ver más abajo)
    });
  } */
  ...
  console.log(obtenerGrupos(data1))
...
console.log(generarResultado('bundesliga')) 
  ...
  function calcularProbabilidades(
    ...
    function returnCasoMasCercano(d) {
      ...
      d.forEach(p => {
        ...
        if (indiceEquipo >= posD && equipo.pts <= ptsD && equipo.diff <= diffD && equipo.gf <= gfD && equipo.gc >= gcD) {
          ...
          gcD = equipo.gc;
          indiceCaso = i;
          console.log('encontro:', indiceCaso, posD, ptsD, diffD, gfD, gcD)
        }
        i++
      })
      masCercano = casos[indiceCaso];
    ...
    equipos.forEach((equipo) => {
      resultado[equipo] = {
        ...
        probabilidad: Number(
          /* d3.format(".3~g") */((clasificaciones[equipo] / simulaciones) * 100).toFixed(1),
        ),
      };
    });
  ...
  let nombre_torneo =
    ress_ratio == "16:9"
      ? "Liga Profesional de Fútbol 2023"
      : ress_ratio == "1:1"
        ? "Liga Profesional de Fútbol 2023"
        : "LPF 2023";
  ...
  let fechas_torneo = new Set(
    torneo
      .filter(
        (d) =>
          d.fecha.split(" ")[1] != fecha_adicional && !d.fecha.includes("1/"),
      )
      .map((d) => d.fecha),
  );
  ...
  let fechas_def = torneo.filter(
    (d) => d.fecha.split(" ")[1] == fecha_adicional,
  );
  ...
  fechas_torneo.forEach((fecha, i) => {
    try {
      let fechas = torneo.filter(
        (d) =>
          d.fecha == fecha &&
          d.dia < torneo.filter((d) => d.fecha == fechas_torneo[i + 1])[0].dia,
      );
      fechas.forEach((d) => {
        Object.assign(d, { fecha2: d.fecha });
      });
      fechas_pospuestas.push(fechas);
      let pendiente = torneo.filter(
        (d) =>
          d.fecha == fecha &&
          d.dia > torneo.filter((d) => d.fecha == fechas_torneo[i + 1])[0].dia,
      );
  ...
  fechas_pospuestas.forEach((d, i) => {
    if (
      d.map((e) => e.fecha2)[0] == "Fecha Post." &&
      d.map((e) => e.fecha2).length == clubes.size / 2
    ) {
  ...
  clubes.forEach((club) => {
    ...
    filter_clubes.forEach((d, i) => {
      if (d.fecha.includes(fecha_adicional)) {
        ...
        let may_deducted = deducted.filter(
          (e) => e.name == d.name && e.dia == d.dia,
        )[0];
        pts_deducted = 0;
        if (may_deducted) {
        ...
        pts_away =
          d.l_or_v == "V" && d.goles !== not_played_yet
            ? pts_away + d.goles
            : pts_away + 0;
        pts = pts + d.pts;
        goles = goles + d.goles;
          ...
          racha1:
            final_list1.filter((d) => d.vs != "none")[i + 1].name != d.name
              ...
              : final_list1.filter((d) => d.vs != "none")[i + 1].racha < d.racha
                ? d.racha
                : final_list1.filter((d) => d.vs != "none")[i + 1].name !=
                    d.name
                  ? d.racha
                  : 0,
        });
